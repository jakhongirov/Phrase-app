...
function UsersPage() {
	...
	React.useEffect(() => {
		...
		fetch(
			process.env.REACT_APP_API_URL + '/users/search?q=o&page=1&limit=20',
			requestOptions,
		)
						...
						<div className='user-page__inner'>
							<div className='users-page__content'>
								<button className='user-page__inner-button'>
									<img
										className='user-page__inner-img'
										src={arrowLeftBtn}
								...
								<span className='user-page__inner-span'>
									{' '}
									{user.length}{' '}
								</span>
							</div>
								...
								<ul className='user-page__main-list'>
									{user.length > 0 &&
										user.map((row) => (
											<li
												key={row.id}
												className='user-page__main-item'>
												<button
													className='user-page__main-button'
													onClick={() =>
														navigate('/user/' + row.id)
													}>
													<div className='user-page__main-img'>
														<img
															src={profilUser}
													...
													<div className='user-page__main-inner'>
														<h3 className='user-page__main-heading'>
															{row.fullname}
														</h3>
														<span className='user-page__main-span'>
															15*****25
														</span>
													</div>
												</button>
